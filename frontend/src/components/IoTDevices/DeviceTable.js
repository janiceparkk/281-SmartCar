...
function DeviceTable({ devices, onRowClick }) {
  ...
  const columns = useMemo(
    () => [
      { Header: "Device ID", accessor: "device_id", width: "12%", align: "left" },
      { Header: "Type", accessor: "type", width: "15%", align: "left" },
      { Header: "Car", accessor: "car", width: "18%", align: "left" },
      { Header: "Status", accessor: "status", width: "10%", align: "center" },
      { Header: "Firmware", accessor: "firmware", width: "15%", align: "left" },
      { Header: "Last Heartbeat", accessor: "last_heartbeat", width: "20%", align: "left" },
    ],
    []
  );
  ...
  const rows = useMemo(
    () =>
      devices.map((device) => ({
        ...
        car: (
          <MDTypography variant="caption" color="text" fontWeight="regular">
            {device.car_id ? `Car ${device.car_id} - ${device.car_model || 'Unknown'}` : "N/A"}
          </MDTypography>
        ),
        status: (
        ...
        last_heartbeat: <LiveHeartbeat lastHeartbeat={device.last_heartbeat} />,
