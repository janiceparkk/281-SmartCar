...
function TelemetryMetricCard({ title, value, unit, color, trend }) {
  ...
  return (
    <Card>
      <MDBox p={2}>
        <MDTypography variant="caption" color="text" fontWeight="regular" textTransform="uppercase">
          {title}
        </MDTypography>
        <MDBox display="flex" alignItems="center" mt={1}>
          <MDTypography variant="h4" fontWeight="bold" color={color || "dark"}>
            ...
            {value !== null && value !== undefined && unit && (
              <MDTypography variant="button" fontWeight="regular" color="text" component="span">
                {" "}
                {unit}
              </MDTypography>
            )}
          ...
          {trend !== null && trend !== undefined && trend !== 0 && (
            <MDBox ml={1}>
              <MDTypography variant="button" color={getTrendColor()} fontWeight="medium">
