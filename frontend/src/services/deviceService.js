const deviceService = {
	...
	async getDeviceStatus(deviceId) {
		try {
			...
			if (!response.ok) {
				throw new Error(
					data.message || "Failed to fetch device status"
				);
			}
	...
	async getFleetHealth() {
		try {
			const token = localStorage.getItem("token");
			const response = await fetch(
				`${process.env.REACT_APP_API_URL}/devices/fleet/health`,
				{
					method: "GET",
					headers: {
						"Content-Type": "application/json",
	...
	async getDeviceDiagnostics(deviceId) {
		try {
			const token = localStorage.getItem("token");
			const response = await fetch(
				`${process.env.REACT_APP_API_URL}/devices/${deviceId}/diagnostics`,
				{
					method: "GET",
					headers: {
						"Content-Type": "application/json",
			...
			if (!response.ok) {
				throw new Error(
					data.message || "Failed to fetch device diagnostics"
				);
			}
