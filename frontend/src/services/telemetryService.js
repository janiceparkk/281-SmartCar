const telemetryService = {
	async getTelemetryData(deviceId, options = {}) {
		try {
			...
			if (!response.ok) {
				throw new Error(
					data.message || "Failed to fetch telemetry data"
				);
			}
	...
	async submitTelemetry(deviceId, telemetryData) {
		try {
			...
			const response = await fetch(
				`${process.env.REACT_APP_API_URL}/devices/${deviceId}/telemetry`,
				{
					method: "POST",
					headers: {
						"Content-Type": "application/json",
			...
			if (!response.ok) {
				throw new Error(
					data.message || "Failed to submit telemetry data"
				);
			}
	...
	formatTelemetryForChart(telemetryData) {
		...
		return {
			...
			signalStrengthData: telemetryData.map(
				(entry) => entry.signal_strength
			),
		};
	},
};
