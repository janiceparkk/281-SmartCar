const commandService = {
	async sendCommand(deviceId, commandData) {
		try {
			...
			const response = await fetch(
				`${process.env.REACT_APP_API_URL}/devices/${deviceId}/commands`,
				{
					method: "POST",
					headers: {
	...
	async getCommandStatus(deviceId, commandId) {
		try {
			const token = localStorage.getItem("token");
			const response = await fetch(
				`${process.env.REACT_APP_API_URL}/devices/${deviceId}/commands/${commandId}/status`,
				{
					method: "GET",
					headers: {
						"Content-Type": "application/json",
	...
	async pollCommandStatus(
		...
		for (let attempt = 0; attempt < maxAttempts; attempt++) {
			try {
				const response = await this.getCommandStatus(
					deviceId,
					commandId
				);
