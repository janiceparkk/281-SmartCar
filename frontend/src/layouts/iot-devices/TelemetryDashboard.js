...
function TelemetryDashboard() {
  ...
  const [chartData, setChartData] = useState({ labels: [], batteryData: [], temperatureData: [], signalStrengthData: [] });
  ...
  const fetchTelemetryData = useCallback(async () => {
    try {
      ...
      setTelemetryData([]);
      setChartData({ labels: [], batteryData: [], temperatureData: [], signalStrengthData: [] });
    } finally {
      setLoading(false);
    }
  }, [deviceId]);
  ...
  const latestData = telemetryData.length > 0 ? telemetryData[telemetryData.length - 1] : null;
                ...
                {telemetryData.length === 0 ? (
                  <MDBox py={6} textAlign="center">
                    ...
                    <MDTypography variant="caption" color="text" display="block" mt={1}>
                      Telemetry data will appear here once the device starts sending data.
                    </MDTypography>
                  </MDBox>
                ) : (
                  <Grid container spacing={3}>
                    ...
                    {chartData.signalStrengthData.length > 0 && (
                      <Grid item xs={12} lg={6}>
                        <TelemetryChart
